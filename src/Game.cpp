...
void Game::Update(float dt) {
    ...
    state.world->my_point_light->Update(dt);
}

void Game::Render(const std::unique_ptr<Camera>& current_camera) {
    ...
    switch (event.type) {
        ...
        case SDL_KEYDOWN:
            ...
            if (event.key.keysym.sym == SDLK_TAB) {
                state.world->mouse_control = !state.world->mouse_control;
                // state.world->me->ToggleMouseControl();
            }
            if (event.key.keysym.sym == SDLK_F11) {
                // Toggle fullscreen switch
                auto fullscreen_flag = SDL_WINDOW_FULLSCREEN_DESKTOP;
                bool is_current_fullscreen = SDL_GetWindowFlags(state.window->handler) & fullscreen_flag;

                if (is_current_fullscreen) {
                    SDL_SetWindowFullscreen(state.window->handler, 0);
                    SDL_SetWindowSize(state.window->handler, state.window->last_width, state.window->last_height);
                } else {
                    SDL_GetWindowSize(state.window->handler, &state.window->last_width, &state.window->last_height);
                    SDL_SetWindowFullscreen(state.window->handler, fullscreen_flag);
                }
            }
            break;
...
void Game::OnKeyDownEvent(const SDL_KeyboardEvent& e) {
    switch (e.keysym.sym) {
        case SDLK_f:
            state.world->my_point_light->enable = !state.world->my_point_light->enable;
            break;
        case SDLK_x:
